 ...
 * of episodes in order to get an estimate of the quality of what has been
 * learned so far.
 * 
 * The experiment estimates statistics such as the mean and standard deviation
 * of the return gathered by the policy and writes those to a comma-separated
 * value file called results.csv.
 * 
 * This experiment also shows off some other features that can be achieved
 * easily through the RL-Glue env/agent messaging system by freezing learning
 * (described above), having the environment start in specific starting states,
 * and saving and loading the agent's value function to/from a binary data file.
 * 
 * @author Brian Tanner
 */
public class SampleExperiment {

	private void saveResultsToCSVFile(final Vector<evaluationPoint> results,
	...
	class evaluationPoint {
		...
		public evaluationPoint(final double mean, final double standardDeviation) {
			this.mean = mean;
	 ...
	 * episodes.
	 */
	void printScore(final int afterEpisodes, final evaluationPoint theScore) {
		System.out.printf("%d\t\t%.2f\t\t%.2f\n", afterEpisodes, theScore.mean,
				theScore.standardDeviation);
	}
	...
	void offlineDemo() {
		...
		System.out
				.println("The results of this experiment have been saved to a"
	...
	public void runExperiment() {
		...
		System.out
				.println("Telling the environment to use fixed start state of 2,3.");
		RLGlue.RL_env_message("set-start-state 2 3");

		RLGlue.RL_start();

		System.out
				.println("Telling the environment to print the current state to the screen.");
