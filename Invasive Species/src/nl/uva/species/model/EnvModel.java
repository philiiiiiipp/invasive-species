            ...
            for (int i = 0; i < habitats.length; ++i) {
                ...
                switch (habitats[i]) {
                    case Utilities.HABITAT_EMPTY:
                        ...
                        // reward += Math.sqrt(habitatsEmpty[habitatIndex]) + Math.sqrt(1 -
                        // habitatsEmpty[habitatIndex]);
                        // reward += (habitatsEmpty[habitatIndex] > 1 / 3 ? 1 : 0);
                        // reward += habitatsEmpty[habitatIndex];
                        break;
                    case Utilities.HABITAT_NATIVE:
                        x = habitatsNative[habitatIndex];
                        // reward += Math.sqrt(habitatsNative[habitatIndex]) + Math.sqrt(1 -
                        // habitatsNative[habitatIndex]);
