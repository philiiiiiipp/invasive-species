...
public class Console {
	...
	/**
	 * Starts a new console while executing the specified command within a
	 * shell.
	 * 
	 * @param command
	...
	public Console(final String... command) {
		...
		// Wait for the process to be started (doesn't require command to be
		// finish)
	...
	public void exec(final String command) {
		if (!mRunning) {
			System.out.println(mPrefix
					+ "Cannot execute a command in a stopped console");
		}
	...
	private class ProcessRunner implements Runnable {
		...
		public void run() {
			try {
				...
				mOutput = new BufferedWriter(new OutputStreamWriter(
