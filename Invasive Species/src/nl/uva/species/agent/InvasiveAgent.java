...
public class InvasiveAgent implements AgentInterface {
	...
	private final double sarsa_stepsize = 0.1;
	private final double sarsa_epsilon = 0.1;
	...
	/**
	 * Choose an action e-greedily from the value function and store the action
	 * and observation. Update the valueFunction entry for the last state,action
	 * pair.
	 * 
	 * @param reward
	 * @param observation
	...
	public Action agent_step(final double reward, final Observation observation) {
		...
		List<Integer> lastActionList = Arrays.asList(ArrayUtils
				.toObject(lastAction_));
		int lastActionIdx = this.all_allowed_actions.get(lastStateId).indexOf(
				lastActionList);
		...
		List<Integer> newActionList = Arrays.asList(ArrayUtils
				.toObject(newAction));
		double Q_sa = this.Q_value_function.get(lastStateId).get(lastActionIdx);
		double Q_sprime_aprime = this.Q_value_function.get(
				Utilities.getStateId(newState)).get(
				this.all_allowed_actions.get(Utilities.getStateId(newState))
						.indexOf(newActionList));
		double new_Q_sa = Q_sa + this.sarsa_stepsize
				* (reward + this.sarsa_gamma * Q_sprime_aprime - Q_sa);
		if (!this.policyFrozen) {
			this.Q_value_function.get(Utilities.getStateId(lastState)).set(
					this.all_allowed_actions.get(
							Utilities.getStateId(lastState)).indexOf(
							lastActionList), new_Q_sa);
		}
		Action returnAction = new Action();
		returnAction.intArray = newAction;
	...
	public void agent_end(final double reward) {
		...
		List<Integer> newActionList = Arrays.asList(ArrayUtils
				.toObject(lastAction_));
		double Q_sa = this.Q_value_function
				.get(Utilities.getStateId(lastState)).get(
						this.all_allowed_actions.get(
								Utilities.getStateId(lastState)).indexOf(
								newActionList));
	 ...
	 * @return
	 */
	private int[] egreedy(final int[] theState) {
		int stateId = Utilities.getStateId(theState);
		if (this.Q_value_function.isEmpty()
				|| !this.Q_value_function.containsKey(stateId)) {
			this.all_allowed_actions.put(stateId, Utilities.getActions(
					theState, this.nbrReaches, this.habitatSize));
			Vector<Double> vector = new Vector<Double>(Collections.nCopies(
					this.all_allowed_actions.get(stateId).size(), 0.0));
			this.Q_value_function.put(stateId, vector);
		}
		int index;
		if (!this.exploringFrozen
				&& this.randGenerator.nextDouble() < this.sarsa_epsilon) {
			index = this.randGenerator.nextInt(this.all_allowed_actions.get(
					stateId).size());
		...
		final int[] result = new int[this.all_allowed_actions.get(stateId)
				.get(index).size()];
		for (int i = 0; i < this.all_allowed_actions.get(stateId).get(index)
				.size(); i++) {
			result[i] = this.all_allowed_actions.get(stateId).get(index).get(i)
					.intValue();
		}
		return result;
	}
