...
public class InvasiveAgent implements AgentInterface {
	...
	/**
	 * Parse the task spec, make sure it is only 1 integer observation and action, and then allocate the valueFunction.
	 * 
	...
	/**
	 * Choose an action e-greedily from the value function and store the action and observation. Update the
	 * valueFunction entry for the last state,action pair.
	 * 
	 * @param reward
	 * @param observation
	 * @return
	...
	public Action agent_step(final double reward, final Observation observation) {
		...
		double Q_sprime_aprime = this.Q_value_function.get(Utilities.getStateId(newState)).get(
				this.all_allowed_actions.get(Utilities.getStateId(newState)).indexOf(newActionList));
		...
		if (!this.policyFrozen) {
			this.Q_value_function.get(Utilities.getStateId(lastState)).set(
					this.all_allowed_actions.get(Utilities.getStateId(lastState)).indexOf(lastActionList), new_Q_sa);
		}
		Action returnAction = new Action();
		returnAction.intArray = newAction;
	...
	/**
	 * This agent responds to some simple messages for freezing learning and saving/loading the value function to a
	 * file.
	 * 
	 * @param inMessage
	 * @return
	...
	/**
	 * Selects a random action with probability 1-sarsa_epsilon, and the action with the highest value otherwise. This
	 * is a quick'n'dirty implementation, it does not do tie-breaking.
	 * 
	 * @param theState
	 * @return
	 */
	...
	/**
	 * This is a trick we can use to make the agent easily loadable. Using this trick you can directly execute the class
	 * and it will load itself through AgentLoader and connect to the rl_glue server.
	 * 
	 * @param args
	 */
